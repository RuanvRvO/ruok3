  ...
  return (
    <SidebarGroup className="p-1.5 min-w-0">
      <div className="flex items-center justify-between px-2 py-1">
        <SidebarGroupLabel className="text-lg font-bold text-slate-900 dark:text-slate-100 tracking-tight p-0">
          Your organisations
        </SidebarGroupLabel>
        <button
          onClick={() => { setShowForm((v) => !v); setError(null); setOrgName(""); }}
          className="flex items-center justify-center w-6 h-6 rounded-md text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors"
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="flex flex-col gap-2 px-2 pb-2 pt-1">
          <input
            type="text"
            value={orgName}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-3 py-2 text-xs font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
            >
              {creating ? "Creating..." : "Create"}
            </button>
            <button
              type="button"
              onClick={() => { setShowForm(false); setOrgName(""); setError(null); }}
              disabled={creating}
